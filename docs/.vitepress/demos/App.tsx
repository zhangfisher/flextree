 ...
 *
 * 显式 import React：文档站 demo 经 vitepress-demo-plugin 编译，
 * classic JSX runtime 下需要 React 标识符在作用域内。
 */
...
export default function App() {
  ...
  if (error)
    return (
      <div className="flextree-demo">
        <pre className="loading">加载失败: {error.message}</pre>
      </div>
    );
  if (!source)
    return (
      <div className="flextree-demo">
        <pre className="loading">正在加载 sql.js (wasm)…</pre>
      </div>
    );
