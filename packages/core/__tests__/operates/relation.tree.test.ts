...
describe("查询节点关系", () => {
  ...
  test("返回后代关系", async () => {
    ...
    expect(await tree.getNodeRelation(a1, a)).toBe(
      FlexTreeNodeRelation.Descendants
    );
    expect(await tree.getNodeRelation(a2, a)).toBe(
      FlexTreeNodeRelation.Descendants
    );
    expect(await tree.getNodeRelation(a3, a)).toBe(
      FlexTreeNodeRelation.Descendants
    );
  });
  test("返回祖先关系", async () => {
    ...
    const a2 = await tree.findNode({ name: "A-2-1" });
    const a3 = await tree.findNode({ name: "A-3-1" });
    expect(await tree.getNodeRelation(a, a1)).toBe(
      FlexTreeNodeRelation.Ancestors
    );
    expect(await tree.getNodeRelation(a, a2)).toBe(
      FlexTreeNodeRelation.Ancestors
    );
    expect(await tree.getNodeRelation(a, a3)).toBe(
      FlexTreeNodeRelation.Ancestors
    );
  });
  test("返回兄弟节点关系", async () => {
    ...
    expect(await tree.getNodeRelation(a1, a2)).toBe(
      FlexTreeNodeRelation.Siblings
    );
    expect(await tree.getNodeRelation(a2, a3)).toBe(
      FlexTreeNodeRelation.Siblings
    );
    expect(await tree.getNodeRelation(a1, a3)).toBe(
      FlexTreeNodeRelation.Siblings
    );
    ...
    expect(await tree.getNodeRelation(a11, a12)).toBe(
      FlexTreeNodeRelation.Siblings
    );
    expect(await tree.getNodeRelation(a12, a13)).toBe(
      FlexTreeNodeRelation.Siblings
    );
    ...
    for (const node of nonSiblings) {
      expect(await tree.getNodeRelation(a1, node)).not.toBe(
        FlexTreeNodeRelation.Siblings
      );
    }
  });
  test("返回同级节点关系", async () => {
    ...
    expect(await tree.getNodeRelation(a1, b2)).toBe(
      FlexTreeNodeRelation.SameLevel
    );
    expect(await tree.getNodeRelation(b2, c3)).toBe(
      FlexTreeNodeRelation.SameLevel
    );
    expect(await tree.getNodeRelation(a1, c3)).toBe(
      FlexTreeNodeRelation.SameLevel
    );
  });
  ...
  test("返回同一棵树关系", async () => {
    ...
    expect(await tree.getNodeRelation(a, b)).toBe(
      FlexTreeNodeRelation.SameTree
    );
  });
