...
describe("查询节点关系", () => {
  ...
  test("返回后代关系", async () => {
    ...
    const a2 = (await tree.findNode({ title: "A-2-1" }))!;
    const a3 = (await tree.findNode({ title: "A-3-1" }))!;
  ...
  test("返回兄弟节点关系", async () => {
    ...
    const a12 = (await tree.findNode({ title: "A-1-2" }))!;
    const a13 = (await tree.findNode({ title: "A-1-3" }))!;
    ...
    const nonSiblings = [
      await tree.getRoot(),
      await tree.findNode({ title: "A" }),
      await tree.findNode({ title: "A-1-1" }),
      await tree.findNode({ title: "A-1-2" }),
      await tree.findNode({ title: "B" }),
      await tree.findNode({ title: "B-1" }),
      await tree.findNode({ title: "B-1-1" }),
    ];
    // a1跟所有非兄弟节点的关系都不是兄弟
  ...
  test("返回同级节点关系", async () => {
    const a1 = (await tree.findNode({ title: "A-1" }))!;
    const b2 = (await tree.findNode({ title: "B-2" }))!;
    const c3 = (await tree.findNode({ title: "C-3" }))!;
  ...
  test("返回同一棵树关系", async () => {
    ...
    const b = (await tree.findNode({ title: "B-1-1" }))!;
