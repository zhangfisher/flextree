...
import type { FlexTreeOptions, IFlexTreeNodeFields } from "../../src";
import { FlexTreeManager, FlexTree, FlexTreeVerifyError } from "../../src";
...
export async function createFlexTree(
  treeId?: any,
  options?: Partial<FlexTreeOptions>
) {
 ...
 * @param srcDb
 */
export async function dumpTree(srcDb: Database, dbFile: string = "tree.db") {
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    if (node.rightValue - node.leftValue === 1) {
      ...
      if (pnodes.length > 0) {
        ...
        if (pnode.level !== node.level - 1) {
          throw new FlexTreeVerifyError(
            `level error ${node.name}(${node.id})`
          );
        } else if (!(node.leftValue > pnode.leftValue)) {
          throw new FlexTreeVerifyError(
            `leftValue error ${node.name}(${node.id})`
          );
        } else if (!(node.rightValue < pnode.rightValue)) {
          throw new FlexTreeVerifyError(
            `rightValue error ${node.name}(${node.id})`
          );
        }
        // 子节点结束
        if (node.rightValue + 1 === pnode.rightValue) {
          ...
          if (pnodes.length > 0) {
            while (
              preNode!.rightValue + 1 === pnodes[pnodes.length - 1]?.rightValue
            ) {
              preNode = pnodes.pop();
              if (pnodes.length === 0) {
                break;
      ...
      if ((node.rightValue - node.leftValue - 1) % 2 !== 0) {
        throw new FlexTreeVerifyError(
          `${node.name}(${node.id}) rightValue - leftValue error `
        );
      }
