 ...
 *
 *
 * - idle: 未加载,如果实例化时未加载节点数据，则状态为not-loaded
 * - loaded:  已加载
...
> {
  ...
  async update(data: Partial<NodeFields>) {
    const nodeData = filterObject(data, (k) => {
      return !(k in this._keyFields) || k === this._keyFields.name;
    });
  ...
  getByPath(
    ...
    const { byField, delimiter } = Object.assign(
      { byField: this._tree.manager.keyFields.name, delimiter: "/" },
      options,
    );
    let [entryNode, entryPath] = getRelNodePath(this as any, path, delimiter) as [
  ...
  _forEachDFS(
    ...
    options?: { includeSelf?: boolean; ignoreErrors?: boolean },
  ) {
