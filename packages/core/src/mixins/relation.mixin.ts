...
> {
  ...
  async getNodeRelation(
    ...
    const node = this.isValidNode(srcNode)
      ? (srcNode as TreeNode)
      : ((await this.getNode(srcNode as NodeId)) as TreeNode);
    const relNode = this.isValidNode(targetNode)
      ? (targetNode as TreeNode)
      : ((await this.getNode(targetNode as NodeId)) as TreeNode);
